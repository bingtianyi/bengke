...
class CrossModalAttention(nn.Module):
    def __init__(self, vis_feat_dim=2048, ir_feat_dim=768, proj_dim=128, dropout=DROP_RATE):
        super().__init__()
...
class DualBackboneDualStream(nn.Module):
    def __init__(self):
        ...
        # 融合模块
        self.fusion = CrossModalAttention(vis_feat_dim=2048, ir_feat_dim=768, proj_dim=128, dropout=DROP_RATE)
    ...
    def _freeze_backbones_partially(self):
        """解冻 ResNet50 的 layer3,4，ConvNeXtV2 的 stages[3]；其余冻结"""
        ...
        for name, param in self.ir_backbone.named_parameters():
            if 'stages.2' in name or 'stages.3' in name:
                ...
                param.requires_grad = False
        print("✅ 解冻主干最后两层 (ResNet layer3,4, ConvNeXt stages[2，3])，其余冻结")
